...
2. JD가 제공되었으면 다음을 수행합니다:
   ...
   - JD를 4가지로 파싱: 회사·도메인 / 핵심 책임 / 우대 키워드 (상위 5~7개) / 시니어리티
   - 각 후보 항목과 JD의 키워드·책임 일치도를 0~100점으로 추정
   - 매칭 점수 + featured 플래그 + 최신성을 종합한 우선순위로 후보를 정렬

3. JD가 없으면 featured + 최신성으로 정렬합니다.

4. 상위 후보(권장 5개)를 표로 사용자에게 제시:

```markdown
## Phase 1: 선별 결과

| # | 프로젝트 | 카테고리 | 기간 | 우선순위 | 매칭 근거 |
|---|---|---|---|---|---|
| 1 | 교육 지원 플랫폼 | outsourced | 2025.05~2026.01 | 92 | featured + JD "MSW" 매칭 |
| 2 | 한국 기술 마켓 | outsourced | 2024.05~ | 88 | featured + JD "Turborepo, FSD" 매칭 |
| 3 | dding-dong | personal | 2026.02 | 75 | featured + AI-Native 어필 |
| 4 | 팀스파르타 (튜터) | experience | 2023.12~2025.05 | 72 | 1년 6개월 경력 + 임팩트 정량 |
| 5 | KC-MIC (FE 엔지니어) | experience | 2022.03~2023.09 | 68 | 1년 6개월 경력 + 레거시 현대화 |
```
...
1. `references/interview-questions.md`를 Read로 로드. 4개 카테고리(앵커링·정량·트레이드오프·실패) 질문 풀에서 항목 특성에 맞게 3~5개를 선별합니다.

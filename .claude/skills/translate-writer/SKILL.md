...
반복 (최대 3회):
  1. 병렬 실행:
     ...
     - translation-verifier 에이전트: 원문 충실도 평가 (3차원 가중치 채점)
       ...
       ⚠️ URL인 경우 반드시 원본 URL을 함께 전달할 것

  2. 결과 판정 (차원별 하드 임계값 + 종합 점수):
     통과 조건:
       - reviewer: 4개 차원 모두 하드 임계값 통과 AND 종합 >= 기준
       - verifier: 3개 차원 모두 하드 임계값 통과 AND 종합 >= 기준
...
- quick: reviewer 종합 8점+ & verifier 종합 8점+ & 모든 차원 하드 임계값 통과, 또는 3회 완료
- thorough: reviewer 종합 8점+ & verifier 종합 8점+ & 모든 차원 하드 임계값 통과, 또는 3회 완료
- perfect: reviewer 종합 9점+ & verifier 종합 9점+ & 모든 차원 하드 임계값 통과, 또는 3회 완료
...
1. ✅ 승인 - 이대로 저장
2. 📝 수정 요청 - 피드백 입력 후 재번역 (체감 점수 1-10 선택사항)
3. 🔧 추가 다듬기 - Phase 3으로 돌아가기
4. ❌ 거절 - 폐기 (체감 점수 1-10 선택사항)
```

> **수정/거절 시 체감 점수 수집**: 사용자가 📝 또는 ❌ 선택 시 "1-10점으로 이 번역의 품질을 평가해주세요 (선택사항)"을 추가로 물어봅니다. 이 점수는 calibration divergence 측정에 사용됩니다.

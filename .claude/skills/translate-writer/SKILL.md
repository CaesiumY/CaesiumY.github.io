...
description: 영어 기술 블로그/문서를 고품질 한국어로 번역하는 6-에이전트 파이프라인. 스타일 학습, 이중 검증(번역투 + 원문 충실도), 문장별 polish까지 자동 수행. 이 스킬은 사용자가 영어 글을 번역해달라고 하거나, URL/파일을 한국어로 옮기고 싶을 때, 번역 품질을 높이고 싶을 때 사용하세요. "번역해줘", "이 글 한국어로", "translate this", "번역 블로그" 등의 요청에 반응합니다. 단순 번역이 아닌 학습 기반 고품질 번역이 필요할 때 /translate-blog 대신 이 스킬을 사용하세요.
---

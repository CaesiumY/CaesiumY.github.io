...
### Step 3: 스타일 가이드 로드
```
.agents/skills/translate-writer/data/style-guide.md
```
- 문체 규칙 확인
- 번역투 회피 패턴 숙지
...
### Step 4: 용어집 로드
```
.agents/skills/translate-writer/data/glossary.md
```
- 원문-번역 매핑 확인
- 일관된 용어 사용 준비
...
대표 5개 패턴 (전체 28개는 `.agents/skills/translate-writer/references/translation-patterns.md` 정본 참조 — 번역 시작 전 한 번 읽어두면 reviewer 감점을 예방할 수 있음):

| 정본 # | 번역투 | 자연스러운 표현 |
|--------|--------|----------------|
| 1 | "~하는 것이 가능하다" | "~할 수 있다" |
| 3 | "~에 의해 ~되다" | 능동태로 변환 |
| 4 | "그것은/이것은" | 구체적 주어 또는 생략 |
| 15 | "~하는 경우" | "~하면", "~할 때" |
| 12 | "~를 수행하다" | "~하다", "~을 실행하다" |

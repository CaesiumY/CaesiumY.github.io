...
### 이미지 소싱 (원격 핫링크 금지)
- 원문의 이미지를 원격 CDN URL로 핫링크(`![](https://...)`)하지 말고, 포스트 폴더에 로컬로 다운로드해 저장 (Astro 이미지 최적화 적용을 위한 프로젝트 규칙, 루트 `CLAUDE.md` "Design Rules" 참조)
- **실제 첫 적용 사례**: Maximizing Claude Code sessions 번역에서 이미지 7개를 원격 링크 대신 로컬 PNG 다운로드로 전환 (2026-08-22)

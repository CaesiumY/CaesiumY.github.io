...
- 다시 그린 도식은 이미지 줄 바로 위에 빈 줄을 사이에 두고 `<!-- 원본 이미지: <원본 이미지 URL> (한국어로 다시 그림, PR #N) -->` 주석을 단다. 이 주석은 에이전트용 `.md` 출력에 그대로 공개되므로 출처 정보만 쓴다. 선례: Claude 5세대 컨텍스트 엔지니어링 번역 4장, Claude Code 세션 번역 7장.
---

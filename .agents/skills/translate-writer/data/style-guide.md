...
- **저자 voice는 글의 성격에 따라 구분**: 기업/팀 명의 기술 블로그(we/our)는 "저희"로 통일. 1인칭 개인 에세이(개인 저자가 자신의 경험·의견을 서술하는 글, I/my)는 "저"로 통일하고 "저희"를 쓰지 않음 — "저희"는 집단(회사·팀)을 가리키므로 개인 저자의 목소리에 쓰면 어색함
- **저자 voice 혼재 문서 (기업 블로그 + 개인 저자)**: 기업 블로그인데 개인 저자가 쓴 글은 1인칭이 문장 단위로 혼재할 수 있음 — 저자 개인 경험 서술("I've written previously")은 "저", 회사를 가리키는 주어("we/our")는 "저희", 독자를 가리키는 주어("you/your")는 "여러분"으로 **문장 단위 3분할** 적용 (실제 사례: Claude 5세대 컨텍스트 엔지니어링 번역, "저는 이전에 다룬 적이 있습니다" / "저희는 이렇게 부릅니다" / "여러분의 CLAUDE.md도 점검해보세요"로 구분, reviewer 개발자맥락 9/10)
...
| "LLM이 참여해/함께" (in-the-loop 약번역) | in-the-loop 관용구 살려 번역 | ❌ "LLM이 함께" → ✅ "LLM을 개발 루프에 두고" |
| "생성된 코드" (수동형) | 주체 명시 능동형 | ❌ "생성된 코드" → ✅ "LLM이 짠 코드" (문맥 따라 주체 풀어쓰기) |
| "사용 시점" (point of use 시간 오독) | 위치로 번역 | ❌ "사용 시점" → ✅ "그 값을 쓰는 자리" |
| "돌아갈 길" (way around 되돌아가는 길 오독) | 우회로로 번역 | ❌ "돌아갈 길" → ✅ "빠져나갈 구멍" (반복 시 "그 구멍"으로 호응) |
| "열린 문자열 표면" (open string surface 직역) | 비유 맥락으로 풀어쓰기 | ❌ "열린 문자열 표면" → ✅ "아무 문자열이나 받아주던 열린 입구" |
| "집다" (reach for 중복·어색) | 습관적 선택 표현 | ❌ "집다" → ✅ "손이 가다" |
| "기본 설정됩니다" (defaults to 기계 어감) | 자연스러운 결과 표현 | ❌ "기본 설정됩니다" → ✅ "별수 없이 가장 손쉬운 길로 빠집니다" |
| "스타일링 패스" (styling pass 음차 모호) | 풀어쓰기 | ❌ "스타일링 패스" → ✅ "스타일을 두 번 입히는 과정" |
| "의견이 담긴 관례" (opinionated convention 어색) | 문서 흐름에 맞춘 의역 | ❌ "의견이 담긴 관례" → ✅ "저희 나름의 방침" (이어지는 "the opinion"→"방침"과 연결 유지) |
| "이것은 ~ 바꿉니다" (This flips... 지시대명사 번역투) | 자연스러운 결과 연결 | ❌ "이것은 ~ 바꿉니다" → ✅ "이렇게 하면 ~ 뒤바뀝니다" |
| "결과물과 함께 사는 것" (live with the output 직역) | 숙어로 번역 | ❌ "결과물과 함께 사는 것" → ✅ "그 결과를 그대로 안고 가다" |
| "그 역" / "그 반대편" (contrapositive 논리 용어) | 논리 함의를 자연스럽게 보존 | ❌ "그 역"(역≠대우 오류) → ✅ "거기서 따라오는 결론" (잿빛 논리 용어 "대우" 지양) |
| "도박" (bet 전략적 베팅 맥락) | 전략적 뉘앙스 보존 | ❌ "도박"(도박장·중독 어감) → ✅ "승부수" (한 문서 일관 적용) |
| "~것들뿐입니다" (make sure 소실) | make sure 능동화 | ❌ "~것들뿐입니다" → ✅ "~만 CI를 통과하도록 해둘 뿐입니다" |
| 이중 주격 비문 (버그가…표현 자체가) | 한쪽을 주제 조사 "는"으로 | ❌ "버그가…표현 자체가" → ✅ "버그는…표현 자체가" |
| "X에는 너무 작다" (too small for X 직역 모호) | 목적어 명시 | ❌ "X에는 너무 작다" → ✅ "X를 담아내기엔 너무 작다" |
| "근본 아이디어는 계속 돌아옵니다" (the one we keep coming back to 주체 반전) | 주체 정확히 설정 | ❌ "아이디어가 돌아옵니다" → ✅ "늘 다시 붙드는 생각은 하나입니다" (주체=저희) |
| "그것은/이것은 ~뿐입니다" 반복 (지시대명사 3회+ 연속) | 주어 구체화 또는 생략, 핵심어 당겨오기 | ❌ "그것은 A일 뿐입니다." "그것은 B일 뿐입니다." (연속 반복) → ✅ 문장마다 주어를 생략하거나 핵심어(A, B)를 문두로 당겨와 다양화 |
| "저를 위해 ~해냅니다" (does X for me 직역) | 결과 중심으로 재구성 | ❌ "모델은 저를 위해 더 나은 작업을 해냅니다" → ✅ "모델은 더 나은 결과를 내놓습니다" |
| "~다는 사실을 확인했습니다" 등 동일 종결 템플릿 반복 (한 문서 3회+) | 발견 동사를 문맥별로 분산 | ❌ 한 문서 안에서 같은 종결 문형을 3회 반복 → ✅ "알게 됐습니다 / 결론에 이르렀습니다 / 깨달았습니다"처럼 분산 (개별로는 자연스러워도 반복되면 기계적으로 읽힘) |
| "~에 관한" (일반, "~에 관해 작성할 때" 외 전반) | 조사나 목적어로 대체 | ❌ 검증 방법에 관한 정보 → ✅ 검증 방법을 다룬 정보 |
| "이 원리를 ~ 적용합니다" (주어-서술어 호응 붕괴) | 원문 주어를 그대로 살리기 | ❌ 이 원리를 도구에도 적용합니다 (주어 없이 능동태만 남아 어색) → ✅ 저희는 도구에도 같은 원리를 적용하고 있습니다 (원문 "we also use it for tools"의 주어 보존) |
| "A, B, 그리고 C" 나열 기계적 반복 (한 문서 6회+) | 접속사 다양화(또는/이나) 또는 구두점 나열로 변주 | ❌ "GPU, TPU, 그리고 하드웨어"류 3항 나열을 한 문서에 6회 반복 → ✅ "GPU, TPU, 또는 모델이 실행되는 어떤 하드웨어든"처럼 접속사를 바꾸거나 일부는 구두점만으로 나열 |

### polish 단계의 함정 (원문 대조 없이 점수만 보고 채택 금지)

**배경**: polish-agent의 점수는 "한국어다움" 단일 축이라, **한정어를 삭제하면 문장이 매끄러워져 점수가 올라가는 구조적 편향**이 있다. Claude 5세대 컨텍스트 엔지니어링 번역에서 최고점 옵션이 탈락시킨 원문 요소:

| 탈락된 원문 요소 | 성격 | 탈락시킨 옵션 점수 |
|-----------------|------|-------------------|
| `as much as possible` ("최대한") | 정도 부사 소실 | 9.9점 |
| `unique` ("고유한") | 수식어 소실 | 9.8점 |
| `just like we did` ("여러분도 저희처럼") | 비교 구문 소실 | 9.8점 |
| `may also be` (가능성 → 단정으로 격상) | 양태 소실 | 9.8점 |
| `we found` (1인칭 주체 소실) | 주체 소실 | 9.7점 |

**규칙**: polish 개선안은 점수 순으로 자동 채택하지 말고, **원문과 대조해 의미가 보존되는 최고점 옵션**을 고를 것.

**모달 평탄화 주의 (뉘앙스 차원 회귀 원인)**: polish 과정에서 `can`(가능)/`should`(권고)가 단정 서술로 일제히 기울면 뉘앙스 점수가 하락한다(실제 사례: 8 → 7). "~할 수 있습니다 과다 → 변형" 규칙을 적용할 때 변형 방향을 전부 단정형으로 몰지 말고, "~하기도 합니다 / ~할 여지가 있습니다"처럼 **양태를 보존하는 변형도 섞을 것**.

**polish-agent(haiku) batch 모드에서 반복 발견된 함정 패턴 (2026-08-22 추가)**: thorough 모드 batch 세션에서도 아래 유형이 재현되었다. 최고점이라도 자동 채택하지 말고 아래 체크리스트로 검증할 것 (Maximizing Claude Code sessions 번역, polish 리포트 observations_for_learner 5건 근거).

| 함정 유형 | 증상 | 검증 방법 |
|-----------|------|-----------|
| 스타일 가이드 위반 옵션 생성 | 기업 블로그 "저희"를 haiku가 "우리는"으로 바꾼 최고점 옵션 생성 | 채택 전 §1 저자 voice 규칙과 대조 |
| 평서체 오염 | 존댓말 문서에 haiku가 평서체(~다) 옵션 생성 | 문체가 문서 전체 톤(존댓말)과 일치하는지 확인 |
| 인접 문장 중복 | 옵션이 바로 다음 문장의 내용을 미리 포함해 적용 시 중복 발생 | 옵션 적용 전 앞뒤 문장과 겹치는 정보가 없는지 확인 |
| 한정어 탈락 재현 | "than you'd think" 같은 한정어가 옵션 전원(3개)에서 탈락 (기존 규칙이 batch에도 그대로 재현) | 옵션 전원이 한정어를 탈락시켰다면 전부 기각하고 원문 유지 |
| 이스케이프 백틱 오염 | 옵션 텍스트에 이스케이프된 백슬래시+백틱 문자가 포함되어 그대로 적용하면 마크다운 파손 | 적용 전 옵션 텍스트에 이스케이프 문자가 섞여 있는지 육안 확인 |
...
| 지시어 없는 예시 나열 (Cards sit on...) | 도입어구 + 인용부호로 예시임을 드러내기 | ❌ "카드는 이 표면 위에 놓입니다. ..." → ✅ "이를테면 이런 결정들이죠. '카드는 …'" |
...
| "way around them" | "빠져나갈 구멍" | 우회로(우회할 방법); "구멍"으로 받아 반복 시 "그 구멍"으로 호응 |
| "the bet" | "승부수" | 전략적 베팅; "도박" 지양. 한 문서 내 일관 |
| "opinionated convention" | "저희 나름의 방침" | 뒤따르는 "the opinion"과 "방침"으로 연결 유지 |
...
- **frontmatter `published` 필드의 연도 오기**: 실제 사례 원문 작성일 2026-07-24가 클리퍼 소스에서 `2001-07-24`로 잘못 표기됨. WebFetch로 원문과 반드시 교차 검증할 것 (2026-07-27, Claude 5세대 컨텍스트 엔지니어링 번역)
- **본문 파일명이 가짜 자동링크로 변환됨**: 실제 사례 텍스트 "CLAUDE.md"가 `[CLAUDE.md](http://claude.md/)`로 변환됨 (존재하지 않는 `.md` TLD). 원문 의도가 아니므로 인라인 코드로 처리하고 링크는 제거
- **인라인 코드 백틱 뒤에 렌더링 잔재 세미콜론이 붙음**: 실제 사례 `` `claude doctor;` `` — 세미콜론은 클리퍼 렌더링 잔재이므로 제거
...
| "This flips who..." | "이렇게 하면 ~ 뒤바뀝니다" | 지시대명사 번역투 제거, 결과 연결로 자연화 |
| "with an LLM in the loop" | "LLM을 개발 루프에 두고" | in-the-loop 관용구 그대로 살리기 |
...
- **CSS 함수/API**: `light-dark()` (CSS 네이티브 함수), StyleX
- **디자인 시스템 고유 컴포넌트**: Box (Orbit의 polymorphic primitive)
...
- **일반 기술 용어**:
  ...
  - design token → 디자인 토큰
  - styling pass → 스타일을 입히는 과정 ("패스" 음차 지양)
- **React/Next.js 용어**:
  - component → 컴포넌트
  - rendering → 렌더링
  - hydration → 하이드레이션
...
- "다형적(polymorphic)" (polymorphic 괄호 병기)

### 일관성 규칙
- 한 문서 내에서 동일 용어는 동일하게 번역
- 용어집에 없는 새 용어 발견 시 용어집 업데이트 제안
- "myth"는 한 문서 내에서 "오해"로 통일. "통념"은 "널리 통용되는 생각"이라는 뜻이 강해 원문의 "이제는 틀린 것으로 밀려났다"는 부정적 함의가 빠지므로 지양
...
### 이미지 alt 텍스트 규칙
- 원문 alt가 비어 있을 때(`![](url)`) 한국어 alt를 새로 작성할 수 있으나, **본문에서 확인되지 않는 정보를 창작하지 말 것**
- 실제 사례: 원문에 없는 도구명("TodoWrite")과 평가어("장황한")를 alt에 넣어 verifier가 T2(의미 추가) Important로 지적 (2026-07-27, Claude 5세대 컨텍스트 엔지니어링 번역)
- 본문에 근거가 있는 최소한의 서술만 사용
- 이미지 직후의 가시 캡션이 alt와 같은 문장을 반복하면 캡션을 유지하고 alt를 비운다(`![](url)`). 클리퍼가 만든 `![x](url)` + `x` 문단은 이 규칙으로 정리한다.
- 최종 Markdown은 `node scripts/check-post-image-accessibility.mjs`를 통과해야 한다.

### 이미지 내용이 이미지 안에만 있을 때 (캡션 추가 규칙)
- **구분**: 위 "이미지 alt 텍스트 규칙"은 본문에 없는 정보를 alt에 창작하는 것을 금지하는 규칙이고, 이 규칙은 반대로 **원문의 실질 정보가 이미지 안 텍스트에만 있고 본문 산문에는 없을 때** 그 정보를 독자에게 전달하는 방법에 관한 규칙
- **적용 시점**: 결론부 요약 카드처럼 원문이 이미지(스크린샷·다이어그램) 안의 텍스트로만 핵심 내용을 전달할 때, 이미지 아래에 한국어 요약/캡션을 추가
- **근거 원칙**: 이미지 안에 실제로 적힌 텍스트에 근거해 전달하는 것은 허용됨 — alt 창작 금지 규칙은 "본문"을 근거로 삼고, 이 규칙은 "이미지 안 텍스트"를 근거로 삼는다는 점에서 서로 다름
- **실제 사례**: Maximizing Claude Code sessions 번역에서 결론부 4개 요소 카드 다이어그램(긴 세션/컨텍스트 과다/모델·effort/캐시 깨뜨리기) 아래에 캡션이 없으면 이미지 텍스트를 읽지 못하는 독자가 핵심 내용을 놓치므로 캡션 추가 (2026-08-22)

### 이미지 소싱과 로컬라이징
- 제3자 원본 이미지는 원격 핫링크를 기본으로 하고, 소유권이나 재배포 범위가 불명확하면 내려받지 않는다.
- 로컬 이미지는 `contents/blog/translation/image-provenance.json`에 `original`, `localized-redraw`, `source-copy` 중 하나로 등록한다.
- `original`은 블로그를 위해 직접 만든 이미지이며 제작 PR 또는 커밋을 기록한다.
- `localized-redraw`는 원본 도식을 참고해 한국어로 다시 만든 결과물이다. 원문, 원본 이미지, 제작 PR, 한국어화 목적을 기록한다. 이는 편집 provenance 기록이지 법적 허락을 의미하지 않는다.
- `source-copy`는 제3자 원본 파일을 그대로 저장한 경우이며, 라이선스 또는 명시적 허락과 증빙을 반드시 기록한다.
- 모든 로컬 이미지는 글 폴더에 두고 `./image.png`로 참조한다. `public/`과 상위 폴더 참조는 금지한다.
- **현재 선례**: Claude 5세대 컨텍스트 엔지니어링 번역(PR #120) 4장과 Claude Code 세션 번역(PR #171) 7장은 한국어 `localized-redraw`로 보존한다 (2026-09-21, #154).
---
...
- **단순 번역** (비추천): "개발자 경험이 중요하면, 신나는 개발자를 가지는 것이 중요합니다"
- **의역 (권장)**: "개발자 경험을 중시한다면, 개발자가 쓰고 싶어하는 도구를 고르는 것도 중요합니다"
- **이유**: 원문의 격언체 표현을 한국어의 자연스러운 논리로 구체화. 독자가 "뭐를 어떻게 해야 하는지" 명확

#### 비유와 감정 표현
- **원문**: "the promise was compelling" (약속가 설득력 있었다 / 혹했다)
- **단순 번역**: "약속은 설득력이 있었습니다"
- **의역 (권장)**: "당시로서는 혹할 만했습니다"
- **이유**: 원문의 심리 상태를 더 생생하게 표현

- **원문**: "honeymoon didn't last" (신혼 시기가 안 늘어났다)
- **의역**: "달콤한 시간은 오래가지 않았습니다"
- **이유**: 관용구로 번역하면 한국 독자에게 훨씬 자연스러움

#### 형용사 나열을 의미 축약으로 변환
- **원문**: "mysterious, confusing directives"
- **단순 번역**: "신비로운, 헷갈리는 지시어"
- **의역 (권장)**: "아리송한 지시어" (한 단어로 축약)
- **이유**: 여러 형용사를 나열하면 번역투처럼 보임. 한국어의 한 단어가 더 자연스러움

#### 부정 조건을 역발상으로 표현
- **원문**: "only worth... in very risk averse environments" (극도로 보수적인 환경에서만 가치 있다)
- **단순 번역**: "극도로 보수적인 환경에서만 가치가 있습니다"
- **의역 (권장)**: "즉, 대부분의 팀에게는 한 번에 밀어붙이는 게 낫습니다. 점진적 전환에 별도 투자할 가치는 극도로 보수적인 환경에서만 있습니다."
- **이유**: 대다수 독자에게 먼저 긍정 메시지를 주고, 예외를 설명하는 구조로 재편성. 실용성 향상

#### 지시어 끊김 보정 (LLM 번역 시 특히 주의)
- **상황**: 원문에서 "Cards sit on this surface. ..." 처럼 예시가 지시어 없이 이어질 때 번역하면 문맥이 끊김
- **권장**: 도입어구("이를테면 이런 결정들이죠.")를 앞에 붙이고, 예시 블록을 인용부호로 묶어 예시임을 드러냄
- **이유**: 한국어 독자는 지시어가 가리키는 대상이 보이지 않으면 끊겨 읽힘

### 의역 사용 시점 판단

| 상황 | 단순 번역 | 의역 | 예시 |
|------|---------|------|------|
| 원문이 격언 형태의 명제 | 부자연스러움 | 권장 | "When DX matters... → 개발자 경험을 중시한다면, 쓰고 싶어하는 도구를" |
| 원문의 비유가 문화적으로 낯섦 | 독자 이해도 낮음 | 권장 | "honeymoon → 달콤한 시간" |
| 원문이 암시적 / 함축이 있음 | 의미 손실 | 권장 | "risk averse only → 대부분은 아닌데, 보수적인..." |
| 형용사가 많이 나열됨 | 번역투 | 권장 | "여러 형용사 → 한 단어 축약" |
| 원문이 기술적으로 명확한 설명 | 좋음 | 불필요 | "라우팅 방식 → 라우팅 방식" |
| 지시어가 가리키는 대상 없이 예시 나열 | 끊겨 읽힘 | 권장 | 도입어구 + 인용부호로 예시임을 드러냄 |
...
| 2026-09-21 | 번역 이미지 출처·로컬라이징 정책 정립 (#154) | 제3자 원본은 핫링크를 기본으로 하고, 직접 만든 이미지·한국어 번안 도식·원본 복제를 `image-provenance.json`에 구분 등록; 기존 PR #120·#171의 11개 번안 도식은 `localized-redraw` 선례로 보존 |

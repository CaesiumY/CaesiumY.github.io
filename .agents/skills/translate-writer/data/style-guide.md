...
### 이미지 소싱과 로컬라이징
- 제3자 원본 이미지는 원격 핫링크를 기본으로 한다. 남의 이미지를 그대로 내려받아 저장하지 않는다 (공식 페이지이거나 누구나 볼 수 있다는 사실은 허락이 아니다).
- 로컬 이미지는 직접 만든 이미지이거나 원본 도식을 한국어로 다시 그린 경우에만 둔다. 글 폴더에 두고 `./image.png`로 참조하며, `public/`과 상위 폴더 참조는 금지한다.
- 다시 그린 도식은 이미지 줄 바로 위에 빈 줄을 사이에 두고 `<!-- 원본 이미지: <원본 이미지 URL> (한국어로 다시 그림: <제작 PR 전체 URL>) -->` 주석을 단다. 공개 `.md`를 읽는 외부 독자도 따라갈 수 있게 PR은 번호가 아니라 전체 URL로 쓴다. 이 주석은 에이전트용 `.md` 출력에 그대로 공개되므로 출처 정보만 쓴다. 선례: Claude 5세대 컨텍스트 엔지니어링 번역 4장, Claude Code 세션 번역 7장.
---
...
| 2026-09-23 | 이미지 출처 레지스트리 폐지 | `image-provenance.json`과 CI 검사를 없애고, 다시 그린 도식의 원본 URL을 이미지 바로 위 HTML 주석으로 옮김 (파일럿 후 확산) |

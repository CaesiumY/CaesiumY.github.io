...
- ⚠️ IMPORTANT: Translated posts may hotlink the source article's images instead, and the choice is made per source, not per post. Localise (download into the post folder) only when redistribution is clearly acceptable — official vendor docs, a permissive licence, or explicit permission from the author. Otherwise hotlink the original URL and accept that the image dies if the source moves it. Record the reason for the call in the PR description. The rule above still applies to any image you do localise: post folder, `./` reference, never `public/`.
- Image `alt` and the visible caption must not repeat the same sentence — a screen reader would announce it twice. Write `alt` for what the image shows, and the caption for why it matters. When a clipper-exported source duplicates them (common with `![x](url)` followed by a bare `x` paragraph), rewrite both instead of copying the duplication through.

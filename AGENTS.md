...
- ⚠️ IMPORTANT: Blog post images live in the same folder as the post markdown (`contents/blog/<post>/image.png`, referenced as `./image.png`) — never in `public/`, which bypasses Astro image optimization, and never through a parent directory (`../`). Use markdown image syntax, not HTML `<img>` tags.
- ⚠️ IMPORTANT: Translated posts hotlink third-party source images by default; never download an unchanged copy of someone else's image (a page being official or freely readable is not permission). A local image in a translation post is only for artwork you made yourself or a diagram you redrew in Korean, and a redrawn diagram gets an HTML comment on its own line directly above the image, separated by a blank line: `<!-- 원본 이미지: <source image URL> (한국어로 다시 그림: <full PR URL>) -->`. That comment is public (it ships verbatim in the agent-facing `/posts/….md` output), so write only source attribution there. The co-location rule above still applies.

...
- ⚠️ IMPORTANT: Translated posts may hotlink the source article's images instead. Hotlinking is the DEFAULT for third-party originals. Local images must be classified in `contents/blog/translation/image-provenance.json` as `original`, `localized-redraw`, or `source-copy`: original blog artwork needs its creation/provenance recorded; a Korean `localized-redraw` needs the source article, source image, provenance PR, and editorial rationale; an unchanged `source-copy` needs a license or explicit permission plus evidence. A page being official, well-known, or freely readable is NOT permission for a source copy, and a `localized-redraw` record is editorial provenance, not a legal permission claim. When ownership or classification is unclear, hotlink that image. The rule above still applies to any local image: keep it beside the post, reference it as `./image.png`, never place it in `public/` or reference a parent directory. CI checks the registry and rejects unregistered local translation images.
...
- Playwright runs against the dev server (port 4321), not the build; CI uses 1 worker with 2 retries. Specs pin real posts — `e2e/fixtures/test-posts.ts`, plus `presentation-mode.spec.ts` which pins two translation posts by URL *and* by their heading/`## 목차`/핵심 요약 structure. Renaming, moving, or restructuring a pinned post breaks the suite; `node scripts/e2e-scope.mjs --check` (in CI) fails first and names the offending literal. ⚠️ Always name a post with its full `/posts/…` URL, never a bare `category/slug`: the scope resolver derives pins from string literals, and a bare slug is indistinguishable from an unrelated string like `image/png` except by heuristics that depend on how many posts the category still has. `--check` rejects bare slugs outright.

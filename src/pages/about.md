...
- **멀티 에이전트 오케스트레이션**: SoC 기반 에이전트 역할 분리, 이중 병렬 검증·피드백 루프로 품질 보장 ([관련 글](https://caesiumy.dev/posts/ai/ai-translation-orchestration))
- **MCP(Model Context Protocol) 통합**: Figma, Linear, Chrome DevTools 연동 ([관련 글](https://caesiumy.dev/posts/ai/mcp-figma-ui-generation))
- **커스텀 Skills 설계**: 반복 워크플로우를 재사용 가능한 스킬로 정의 ([관련 글](https://caesiumy.dev/posts/ai/custom-commands-instead-of-cli-claude-code))
...
- **멀티 에이전트 번역 파이프라인**: 6개 전문 에이전트의 단계별 처리로 번역 품질 체계화 ([관련 글](https://caesiumy.dev/posts/ai/ai-translation-orchestration))

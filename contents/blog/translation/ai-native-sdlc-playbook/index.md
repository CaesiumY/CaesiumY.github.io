...
description: "Anthropic 공식 블로그 번역입니다. 코드 작성이 더는 병목이 아닌 시대에 맞춰 SDLC를 루프로 다시 설계하는 방법을, 여섯 단계에 걸친 플레이와 intent.md, CLAUDE.md, 스킬, 훅, 평가를 중심으로 소개합니다."
...
### 주요 내용
- **병목은 코드 작성에서 그 앞뒤 단계로 옮겨갔습니다.** 구현은 몇 시간으로 줄었지만 계획, 리뷰와 테스트, 배포는 여전히 사람 속도로 돌아갑니다. 한 줄씩 손으로 하는 리뷰는 에이전트의 산출 속도를 따라가지 못하고, 회의와 위원회를 거치는 예외 처리는 거버넌스 비용을 키웁니다.
...
플레이는 단계별로 나열되어 있고 화살표는 도입 순서를 알려줍니다. 이 둘은 같지 않습니다. 주황색 시작 플레이 가운데 아무거나 하나로 시작하면 됩니다(옮긴이: 위 그래프 맨 윗줄의 의도 포착(Capture intent), CLAUDE.md, 피드백 루프(Feedback loop), 훅(Hooks), 플랜 모드(Plan mode)). 이 플레이들은 들어오는 화살표가 없으므로 먼저 해야 할 다른 무언가가 없습니다. 그 외의 플레이는 자기 쪽으로 화살표를 보내는 플레이들을 먼저 도입해야 합니다.
...
누군가 아이디어를 떠올리면 Claude와 브레인스토밍을 하고 마크다운 형식의 프로토 스펙을 만듭니다. 기존 SDLC에서는 제안자가 제품팀 구성원을 설득해, 함께 또는 그 사람에게 맡겨 아이디어를 문서로 정리해야 합니다.
...
증거는 커밋된 `intent.md`이며 작성자, 타임스탬프, 전체 수정 이력이 담겨 있습니다. 이 내용은 intent 저장 공간의 git 히스토리에 기록됩니다. 제품 책임자가 승인하며, 의도를 2단계: 설계로 넘길지 정하는 수락 또는 거절 결정은 머지나 리뷰 종료로 기록됩니다.
...
구현 시작 후의 요구사항 재작업입니다. 같은 변경에서 첫 `plan.md` 커밋보다 날짜가 뒤인 `spec.md` 커밋 수를 셉니다. git log로 바로 확인할 수 있습니다.
...
정책 책임자가 정책 변경을 승인한 시점부터 갱신된 스킬이 머지되기까지의 시간입니다. 스킬 폴더의 PR에서 확인합니다.

**후행 지표**

정책을 인용하는 PR 리뷰 지적 사항입니다. 스킬이 코드를 작성하는 동안 정책을 적용하고 있다면 0에 가까워져야 합니다. 0에 가까워지지 않는다면 스킬이 발동하지 않거나, 스킬의 문구가 공식 정책과 어긋났다는 뜻입니다.
...
리뷰 품질이 유지되는 범위에서 엔지니어 한 명이 동시에 돌리는 세션 수입니다. OpenTelemetry 내보내기 데이터로 집계합니다. 여기에 기다리지 않고 조종하는 데 쓴 하루 시간의 비중을 함께 봅니다.

**후행 지표**

엔지니어 1인당 주간 머지 건수입니다. PR 이력으로 산출한 재작업 비율과 나란히 놓고 해석합니다.
...
피드백 루프를 검증자 서브에이전트(3단계: 구현)와 혼동하면 안 됩니다. 피드백 루프는 작업량만큼 여러 번, 작업 내내 돌아갑니다. 반면 검증자 서브에이전트는 마지막 확인을 구성하는 방법 중 하나로, 세션이 작업을 끝냈다고 판단한 시점에 새 컨텍스트 윈도우로 최종 점검을 실행합니다. 이렇게 하면 코드를 만들어낸 가정에 판정이 물들지 않습니다.
...
3단계: 구현에서 갱신한 `CLAUDE.md` 파일과 정의된 서브에이전트가 필요하고, 리뷰 패스가 문서화된 정책을 강제한다면 스킬도 필요합니다.
...
`allowManagedHooksOnly`는 실행되는 훅이 이 플레이의 승인 게이트뿐이며, 로컬에서는 훅을 추가하거나 교체할 수 없다는 뜻입니다.
...
위 설정은 그대로 복사하라는 권장이 아니라, 각자 상황에 맞게 조정할 출발점으로 보세요. deny를 하나 추가할 때마다 그만큼 기능을 내주게 되므로 적절한 균형은 저장소의 데이터 분류 등급에 따라 달라집니다. 관리형 설정 전용 키를 포함한 모든 키는 설정 레퍼런스에 정리되어 있습니다: [code.claude.com/docs/en/settings](https://code.claude.com/docs/en/settings)

**측정 방법 (훅 자체에 대해)**

**선행 지표**

각 승인 게이트에서 대기하는 시간입니다. 모든 훅 결정은 타임스탬프와 허용 또는 차단 판정과 함께 OpenTelemetry 내보내기에 기록되므로 게이트별 대기 시간을 확인할 수 있습니다.

**후행 지표**

훅 도입 전후로, 게이트 위반이 프로덕션까지 도달한 건수(인시던트 트래커 기준)입니다.
...
CI/CD 파이프라인 로그에서 가져온, 사람을 호출하지 않고 분류를 마친 파이프라인 실패의 비율입니다.

**후행 지표**

DORA(DevOps Research and Assessment) 지표입니다. CI 시스템과 배포 도구가 이미 내보내는 값입니다.
...
지금까지 SDLC의 각 단계에 Claude를 넣는 방법을 다뤘습니다. 어느 단계든 처음 몇 걸음은 사람이 시작해야 했습니다. 하지만 이 단계에서는 Claude가 자율적으로 실행되며 루프를 닫는 데 초점을 둡니다.

예를 들어 상시 실행되는 모니터링 에이전트가 버그 티켓이 생성되는 것을 계기로 `intent.md`를 만들고 요구사항, 계획, 구현, 테스트, 리뷰 단계를 차례로 거칠 수 있습니다. 6단계: 유지보수는 사람 개입 없이 헤드리스로 실행됩니다. 단계 사이마다 독립된 신뢰도 게이트(결정론적 검사 또는 적대적 리뷰 에이전트)가 있어, 이전 단계의 결과물을 계속 진행할지 사람에게 에스컬레이션할지를 판단합니다.
...
**AI 네이티브**  스캔은 연결된 모든 저장소를 대상으로, 사용 가능한 가장 강력한 모델로 일정에 맞춰 실행됩니다. 발견 사항은 누군가 읽기 전에 검증을 거칩니다. 각 발견 사항은 관리 범위 이탈과 같은 방식으로 처리됩니다. PR 하나로 끝나는 수정은 리뷰 게이트를 거치고 더 큰 사안은 `intent.md`가 됩니다. 점검 범위는 첫 스캔이 아니라 가장 최근 스캔 시점을 기준으로 따집니다.
...
이 변화는 사람의 판단을 프로세스의 중심에 두고, 대기업이 요구하는 거버넌스와 규제 요건까지 고려합니다.
...
- [설정 레퍼런스와 우선순위](https://code.claude.com/docs/en/settings) (관리형 설정 전용 키를 모두 포함)
...
이 가이드에 기여해 주신 Jim Blackhurst, Will Steuk, Jamal Arif에게 감사드립니다. 이 가이드는 세 사람의 이전 작업에서 영감을 받았고, 그 작업의 상당 부분을 토대로 삼았습니다.

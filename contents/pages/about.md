...
**프론트엔드 엔지니어 (2022.03 ~ 2023.09, 1년 6개월) · 5인 팀 중 프론트엔드 2인**
...
- **상태관리 최적화**: 화면이 매 프레임 갱신되는 특성상 리더보드처럼 자주 바뀔 필요가 없는 컴포넌트까지 프레임마다 리렌더되던 문제 → Redux Toolkit(RTK) 슬라이스를 **갱신 주기 기준으로 분리**해 구독 범위를 좁힘 — 비실시간 컴포넌트를 프레임 단위 리렌더 대상에서 제외
...
- Astro 기반 정적 기술 블로그 — 배포 후에야 회귀를 발견하던 문제를 GitHub Actions 단일 머지 게이트(의존성 감사 → 린트 → 포맷 → 자작 검증 3종 → 빌드 → E2E 65개 케이스)와 GitHub Pages 자동 배포로 해결 — PR 단계에서 회귀가 자동 검출되고, Dependabot 의존성 갱신도 로컬이 아닌 클린 컨테이너 환경에서 빌드·배포까지 검증

...
- **모노레포 아키텍처 설계와 재정비(Turborepo, FSD)**: 동일 기능에 다른 UI를 배포하는 특성상 유저 모노레포(8+ 앱), 관리자, 대시보드 3개 레포를 Turborepo로 운영하며 공통 로직을 Core 패키지로 추출했습니다. 신규 앱 셋업 시간과 중복 코드가 줄고 공통 수정 1회가 8+ 앱에 일괄 반영됩니다.

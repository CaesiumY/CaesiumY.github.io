...
**Considered Options:** `translation` 태그(오탐 1건. 이 글에서 태그를 떼면 `/tags/translation` 주제 검색에서 빠집니다), 제목 `[번역]` 접두어(디렉터리와 똑같이 18/18 적중. 신호로는 디렉터리를 쓰고, 둘이 서로 맞는지는 `scripts/check-post-classification.mjs`가 CI에서 검사합니다), 스키마의 `canonicalURL`(선언만 되어 있고 실제로 쓰는 글이 없음).

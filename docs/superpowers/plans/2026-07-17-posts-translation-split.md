...
- **DEV vs PROD 카운트**: `ai/ai-native-development-method/index.md`가 `draft: true`라 dev 서버(E2E)에선 자작 27편, 프로덕션 빌드에선 26편이 노출된다. 페이지 수는 양쪽 모두 전체 6·자작 4·번역 3으로 동일하므로, E2E는 페이지 수와 "첫 페이지 8개"로만 단언한다(총 글 수로 단언하지 않는다).
...
test.describe("게시글 번역/자작 탭 분리", () => {
  test("authored 탭: 첫 페이지 8개 카드 + 4페이지", async ({ page }) => {
    await page.goto("/posts/authored");
    await expect(page.locator("#main-content ul > li")).toHaveCount(8);
    await expect(page.getByLabel("Pagination")).toContainText(/1\s*\/\s*4/);
  });

  test("translated 탭: 첫 페이지 8개 카드 + 3페이지", async ({ page }) => {
